...
//go:generate minimock -i github.com/evg555/auth/internal/repository.UserRepository -o ./mocks/user_repository_minimock.go -n UserRepositoryMock
...
type UserRepositoryMock struct {
	...
	funcLog          func(ctx context.Context, method string, user *model.User) (err error)
	inspectFuncLog   func(ctx context.Context, method string, user *model.User)
	afterLogCounter  uint64
	beforeLogCounter uint64
	LogMock          mUserRepositoryMockLog
...
func NewUserRepositoryMock(t minimock.Tester) *UserRepositoryMock {
	...
	m.LogMock = mUserRepositoryMockLog{mock: m}
	m.LogMock.callArgs = []*UserRepositoryMockLogParams{}
...
type mUserRepositoryMockLog struct {
	mock               *UserRepositoryMock
	defaultExpectation *UserRepositoryMockLogExpectation
	expectations       []*UserRepositoryMockLogExpectation

	callArgs []*UserRepositoryMockLogParams
	mutex    sync.RWMutex
}

// UserRepositoryMockLogExpectation specifies expectation struct of the UserRepository.Log
type UserRepositoryMockLogExpectation struct {
	mock    *UserRepositoryMock
	params  *UserRepositoryMockLogParams
	results *UserRepositoryMockLogResults
	Counter uint64
}

// UserRepositoryMockLogParams contains parameters of the UserRepository.Log
type UserRepositoryMockLogParams struct {
	ctx    context.Context
	method string
	user   *model.User
}

// UserRepositoryMockLogResults contains results of the UserRepository.Log
type UserRepositoryMockLogResults struct {
	err error
}

// Expect sets up expected params for UserRepository.Log
func (mmLog *mUserRepositoryMockLog) Expect(ctx context.Context, method string, user *model.User) *mUserRepositoryMockLog {
	if mmLog.mock.funcLog != nil {
		mmLog.mock.t.Fatalf("UserRepositoryMock.Log mock is already set by Set")
	}

	if mmLog.defaultExpectation == nil {
		mmLog.defaultExpectation = &UserRepositoryMockLogExpectation{}
	}

	mmLog.defaultExpectation.params = &UserRepositoryMockLogParams{ctx, method, user}
...
// Inspect accepts an inspector function that has same arguments as the UserRepository.Log
func (mmLog *mUserRepositoryMockLog) Inspect(f func(ctx context.Context, method string, user *model.User)) *mUserRepositoryMockLog {
	if mmLog.mock.inspectFuncLog != nil {
		mmLog.mock.t.Fatalf("Inspect function is already set for UserRepositoryMock.Log")
	}
...
// Return sets up results that will be returned by UserRepository.Log
func (mmLog *mUserRepositoryMockLog) Return(err error) *UserRepositoryMock {
	if mmLog.mock.funcLog != nil {
		mmLog.mock.t.Fatalf("UserRepositoryMock.Log mock is already set by Set")
	}

	if mmLog.defaultExpectation == nil {
		mmLog.defaultExpectation = &UserRepositoryMockLogExpectation{mock: mmLog.mock}
	}
	mmLog.defaultExpectation.results = &UserRepositoryMockLogResults{err}
	return mmLog.mock
}

// Set uses given function f to mock the UserRepository.Log method
func (mmLog *mUserRepositoryMockLog) Set(f func(ctx context.Context, method string, user *model.User) (err error)) *UserRepositoryMock {
	if mmLog.defaultExpectation != nil {
		mmLog.mock.t.Fatalf("Default expectation is already set for the UserRepository.Log method")
	}

	if len(mmLog.expectations) > 0 {
		mmLog.mock.t.Fatalf("Some expectations are already set for the UserRepository.Log method")
	}
...
// When sets expectation for the UserRepository.Log which will trigger the result defined by the following
// Then helper
func (mmLog *mUserRepositoryMockLog) When(ctx context.Context, method string, user *model.User) *UserRepositoryMockLogExpectation {
	if mmLog.mock.funcLog != nil {
		mmLog.mock.t.Fatalf("UserRepositoryMock.Log mock is already set by Set")
	}

	expectation := &UserRepositoryMockLogExpectation{
		mock:   mmLog.mock,
		params: &UserRepositoryMockLogParams{ctx, method, user},
	}
	mmLog.expectations = append(mmLog.expectations, expectation)
	return expectation
}

// Then sets up UserRepository.Log return parameters for the expectation previously defined by the When method
func (e *UserRepositoryMockLogExpectation) Then(err error) *UserRepositoryMock {
	e.results = &UserRepositoryMockLogResults{err}
	return e.mock
}

// Log implements repository.UserRepository
func (mmLog *UserRepositoryMock) Log(ctx context.Context, method string, user *model.User) (err error) {
	mm_atomic.AddUint64(&mmLog.beforeLogCounter, 1)
	defer mm_atomic.AddUint64(&mmLog.afterLogCounter, 1)

	if mmLog.inspectFuncLog != nil {
		mmLog.inspectFuncLog(ctx, method, user)
	}

	mm_params := &UserRepositoryMockLogParams{ctx, method, user}
	...
	if mmLog.LogMock.defaultExpectation != nil {
		...
		mm_got := UserRepositoryMockLogParams{ctx, method, user}
		if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmLog.t.Errorf("UserRepositoryMock.Log got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}

		mm_results := mmLog.LogMock.defaultExpectation.results
		if mm_results == nil {
			mmLog.t.Fatal("No results are set for the UserRepositoryMock.Log")
		}
		return (*mm_results).err
	}
	if mmLog.funcLog != nil {
		return mmLog.funcLog(ctx, method, user)
	}
	mmLog.t.Fatalf("Unexpected call to UserRepositoryMock.Log. %v %v %v", ctx, method, user)
	return
}

// LogAfterCounter returns a count of finished UserRepositoryMock.Log invocations
func (mmLog *UserRepositoryMock) LogAfterCounter() uint64 {
	return mm_atomic.LoadUint64(&mmLog.afterLogCounter)
}

// LogBeforeCounter returns a count of UserRepositoryMock.Log invocations
func (mmLog *UserRepositoryMock) LogBeforeCounter() uint64 {
	return mm_atomic.LoadUint64(&mmLog.beforeLogCounter)
}

// Calls returns a list of arguments used in each call to UserRepositoryMock.Log.
// The list is in the same order as the calls were made (i.e. recent calls have a higher index)
func (mmLog *mUserRepositoryMockLog) Calls() []*UserRepositoryMockLogParams {
	mmLog.mutex.RLock()

	argCopy := make([]*UserRepositoryMockLogParams, len(mmLog.callArgs))
...
// the number of defined expectations
func (m *UserRepositoryMock) MinimockLogDone() bool {
...
func (m *UserRepositoryMock) MinimockLogInspect() {
	for _, e := range m.LogMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to UserRepositoryMock.Log with params: %#v", *e.params)
		}
	}
	...
	if m.LogMock.defaultExpectation != nil && mm_atomic.LoadUint64(&m.afterLogCounter) < 1 {
		if m.LogMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to UserRepositoryMock.Log")
		} else {
			m.t.Errorf("Expected call to UserRepositoryMock.Log with params: %#v", *m.LogMock.defaultExpectation.params)
		}
	}
	// if func was set then invocations count should be greater than zero
	if m.funcLog != nil && mm_atomic.LoadUint64(&m.afterLogCounter) < 1 {
